...
def outplot(Matrix, age_lst, SqFtLiving, Bedrooms):
    ...
    ax1.plot(Est_mean, 'k^-', label='Mean $ Estimate')
    ax1.plot(Est_min, '--r', label='Min $ Estimate')
    ax1.plot(Est_max, '--r', label='Max $ Estimate')
    ...
    ax1.set_xticklabels(zipcodes_str +['']*3)
    ax2.set_xticks(range(len(zipcodes_str)+3), zipcodes_str +['']*3)
    ax2.set_xticklabels(zipcodes_str +['']*3)
    plt.tight_layout()
    #plt.show()
    return f

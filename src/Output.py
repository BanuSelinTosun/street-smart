...
def outplot(Matrix, age_lst, SqFtLiving, Bedrooms):
    ...
    f, (ax1, ax2) = plt.subplots(2, 1, sharex=True, figsize=(18, 18/4), facecolor=(0,0,0,0.5))
    ax1.plot(Est_mean, 'rd', label='Mean $ Estimate', markersize=13)
    ax1.plot(Est_min, 'bd', label='Min $ Estimate', markersize=8)
    ax1.plot(Est_max, 'bd', label='Max $ Estimate', markersize=8)
    ax1.grid(color='white', linestyle='--', linewidth=1)
    ax1.set_ylim(min(Est_min)-1, max(Est_max)+1)
    ax1.set_yticks(range(int(min(Est_min)-1.1), int(max(Est_max)+1.1), 2))
    ax1.set_yticklabels(range(int(min(Est_min)-1.1), int(max(Est_max)+1.1), 2), fontweight='bold', color='white')
    ax1.set_ylabel('Estimated $ X 100K', fontweight='bold', color='white')
    ax1.legend(framealpha=0)
    ax1.set_facecolor((1,1,1,0))
    ax2.plot(ES_Rate, 'gs', label='Elementary School', markersize=13)
    ax2.plot(MS_Rate, 'r^', label='Middle School', markersize=13)
    ax2.plot(HS_Rate, 'bo', label='High School', markersize=13)
